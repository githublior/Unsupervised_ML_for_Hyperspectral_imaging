%more_index_to_rm
function  [PL,extracted] = temp_comon_area(PL,regions, local_indx_in_this_window, compress_percentage, same_size_percentage,min_dist);
...
for i=1:s_loc
    ...
    for j=i+1:s_loc
        ...
        full = cat(1,local_mserR(i).PixelList,local_mserR(j).PixelList);
        [f1,~] = size(full);
        common = unique(full,'rows');
        [c1,~]= size(common);
        
        distance_matrix(i,j)= abs(x_i - x_j) + abs(y_i - y_j);
        
        %if they have pixels in common
        if f1  > c1  && distance_matrix(i,j)>0
