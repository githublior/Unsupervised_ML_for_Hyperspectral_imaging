...
function [W_PL , f_1_r_regions] = filter_doublon_inter(MserRegions,plot_f_1,bg,compress_percentage, same_size_percentage,min_dist,area_size_row,area_size_col,slide_by_raw, slide_by_col,min_range,max_range,close_enough);
%{
...
for raw_wind=min(area_size_row, slide_by_raw): slide_by_raw / 2 : area_size_row
    ...
    for col_wind=min(area_size_col,slide_by_col):slide_by_col/2:area_size_col
        ...
        if nbr_regions_in_window >0         
            ...
            % apply the filter 
            [ more_W_PL, indx_to_rm] = doublon_filter( MserRegions,local_indx_in_this_window, compress_percentage, same_size_percentage,min_dist,indx_to_rm,close_enough,bg, 'inter');
